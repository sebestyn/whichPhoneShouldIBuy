...
//DATABASE
mongoose.connect("mongodb+srv://sebestyn:sebestyn@cluster0-uek7j.gcp.mongodb.net/whichPhone?retryWrites=true&w=majority", { useNewUrlParser: true, useUnifiedTopology: true }, () => {
			...
			data.forEach((telefon) => {
				...
				try {
					...
					if (!isNaN(ram_array[ram_array.indexOf("RAM") - 2])) {
						ram = ram_array[ram_array.indexOf("RAM") - 2] + ram_array[ram_array.indexOf("RAM") - 1];
					} else {
				...
				try {
					...
					let battery = battery_array[battery_array.indexOf("mAh") - 1];
				...
				//SIM
				try {
					if (telefon.sim.split(" ").includes("Dual") || telefon.sim.split(" ").includes("dual")) {
				...
				//SCREEN TYPE
				try {
					if (telefon.type.split(" ").includes("AMOLED") || telefon.type.split(" ").includes("Amoled")) {
						phone_important.screenType = "AMOLED";
					} else if (telefon.type.split(" ").includes("OLED") || telefon.type.split(" ").includes("Oled")) {
				...
				try {
					...
					let resolution = resolution_array[resolution_array.indexOf("pixels") - 3] + resolution_array[resolution_array.indexOf("pixels") - 2] + resolution_array[resolution_array.indexOf("pixels") - 1];
					if (!resolution) {
						resolution = resolution_array[resolution_array.indexOf("pixels,") - 3] + resolution_array[resolution_array.indexOf("pixels,") - 2] + resolution_array[resolution_array.indexOf("pixels,") - 1];
					}
				...
				try {
					if (telefon.price.split(" ").includes("EUR")) {
						phone_important.price = telefon.price.split(" ")[telefon.price.split(" ").indexOf("EUR") - 1];
					} else {
...
function calculatePhonePoint(wantSpecs, phone) {
	...
	//PRICE
	if ((wantSpecs[1] == "300" && Number(phone.price) < 320) || (wantSpecs[1] == "300-600" && Number(phone.price) < 620 && Number(phone.price) > 280) || (wantSpecs[1] == "600" && Number(phone.price) > 500)) {
		osszPont += priceP;
	}
	//SIZE
